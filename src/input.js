const { isOption, isNumber, isDash } = require("./util.js");

const areOptionCountTogether = function(args) {
  return args.length >= 3 && isDash(args[0]) && isOption(args[1]);
};
...
const areOptionCountSeparated = function(args) {
  return args.length == 2 && isDash(args[0]) && isOption(args[1]);
};
...
const isOnlyCountGiven = function(args) {
  return args.length >= 2 && isNumber(args.slice(1));
};
